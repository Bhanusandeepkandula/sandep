...
/** Marker localStorage key per-uid of the last auto-generation date (ISO YYYY-MM-DD).
 *  Used to ensure auto-generation fires at most once per cycle end. */
const AUTO_GEN_LS_KEY = "track_spending_report_auto_gen";
function autoGenKey(uid) { return `${AUTO_GEN_LS_KEY}_${uid || "anon"}`; }

/** Last day of the current cycle given a cycleDay:
 *  - cycleDay <= 1 → last day of current calendar month
 *  - cycleDay > 1  → (cycleDay - 1) of the next calendar month if today >= cycleDay,
 *                    otherwise (cycleDay - 1) of the current month.
 */
function cycleEndDate(now, cycleDay) {
  const d = now.getDate();
  if (!cycleDay || cycleDay <= 1) {
    return new Date(now.getFullYear(), now.getMonth() + 1, 0);
  }
  if (d >= cycleDay) {
    return new Date(now.getFullYear(), now.getMonth() + 1, cycleDay - 1);
  }
  return new Date(now.getFullYear(), now.getMonth(), cycleDay - 1);
}

/**
 * Auto-generate the spending report only in two cases:
 *   1) User has NEVER generated a report for any month (first-time onboarding).
 *   2) Today is the final day of the current cycle AND we have not already
 *      auto-generated a "final" report today.
 * Visiting the Reports tab by itself must NOT trigger OpenAI calls (token cost).
 */
function shouldAutoGenerate(uid, currentMk, cycleDay) {
  const now = new Date();
  const history = loadHistory(uid);
  const everGenerated = Object.values(history || {}).some((e) => e?.report?.generatedAt);
  const todayIso = isoStr(now);
  const lastAutoIso = (() => {
    try { return localStorage.getItem(autoGenKey(uid)) || ""; } catch { return ""; }
  })();

  if (!everGenerated) return { run: true, reason: "first-time" };

  const endIso = isoStr(cycleEndDate(now, cycleDay));
  if (todayIso === endIso && lastAutoIso !== todayIso) {
    // Avoid regenerating if user already finalized the cycle manually today.
    const todayEntry = loadMonthReport(uid, currentMk);
    const genToday = todayEntry?.report?.generatedAt
      && isoStr(new Date(todayEntry.report.generatedAt)) === todayIso;
    if (genToday) return { run: false };
    return { run: true, reason: "cycle-end" };
  }

  return { run: false };
}

function markAutoGenerated(uid) {
  try { localStorage.setItem(autoGenKey(uid), isoStr(new Date())); } catch { /* quota */ }
}
...
export function SpendingReport({
  ...
  monthlyBudgetTotal,
  uid,
  /** Salary/billing cycle start day (1-28). `1` = calendar month.
   *  Used to pick the auto-generation end-of-cycle date. */
  cycleDay = 1,
  px = 20,
  splitAnalytics = null,
  ...
  const activeTxs = useMemo(() => {
    ...
    // If a snapshot exists, prefer it; otherwise fall back to live txs so
    // that historical months without a saved snapshot still show their
    // transactions/analytics in the History detail view.
    if (viewingEntry?.snapshot?.txs?.length) return viewingEntry.snapshot.txs;
    const prevMk = prevMonthKey(viewingMk);
    return txs.filter((t) => {
      const d = (t.date || "").slice(0, 7);
      return d === viewingMk || d === prevMk;
    });
  }, [isViewingCurrent, txs, viewingEntry, viewingMk]);

  const activeFixed = useMemo(() => {
    if (isViewingCurrent) return fixedExpenses;
    return (viewingEntry?.snapshot?.fixedExpenses?.length ? viewingEntry.snapshot.fixedExpenses : fixedExpenses);
  }, [isViewingCurrent, fixedExpenses, viewingEntry]);

  const activeFixedTotal = useMemo(() => {
    if (isViewingCurrent) return fixedTotal;
    return (viewingEntry?.snapshot?.fixedTotal ?? fixedTotal ?? 0);
  }, [isViewingCurrent, fixedTotal, viewingEntry]);

  const activeBudgetTotal = useMemo(() => {
    if (isViewingCurrent) return monthlyBudgetTotal;
    return (viewingEntry?.snapshot?.monthlyBudgetTotal ?? monthlyBudgetTotal ?? 0);
  ...
  useEffect(() => {
    ...
    const { run } = shouldAutoGenerate(uid, currentMk, cycleDay);
    if (run) generateReport();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [uid, cycleDay, txs.length > 0, currentMk]);
      ...
      {expanded && (
        <>
          ...
          {weeksBarData.length > 0 && (() => {
            const weeklyThis = thisAnalytics.weeks.reduce((s, v) => s + (v || 0), 0);
            const weeklyLast = lastAnalytics.weeks.reduce((s, v) => s + (v || 0), 0);
            const weeklyChg = percentChange(weeklyThis, weeklyLast);
            return (
            <div style={{ ...card, marginBottom: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, gap: 8 }}>
                <div style={{ fontSize: 14, fontWeight: 700 }}>Weekly Pattern</div>
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" }}>
                  <span style={{ fontSize: 10, color: T.sub }}>vs {lastAnalytics.label}</span>
                  <ChangeBadge val={weeklyChg} />
                </div>
              </div>
          ...
          {dailyLineData.length > 0 && (() => {
            const dailyThis = thisAnalytics.total;
            const dailyLast = lastAnalytics.total;
            const dailyChg = percentChange(dailyThis, dailyLast);
            return (
            <div style={{ ...card, marginBottom: 12 }}>
          ...
          {/* Transactions list for historical months — includes BOTH
               discretionary and mandatory, sorted newest → oldest. */}
          {!isViewingCurrent && (() => {
            const monthTxs = (activeTxs || []).filter((t) => (t.date || "").slice(0, 7) === viewingMk);
            if (monthTxs.length === 0) {
              return (
                <div style={{ ...card, marginBottom: 12, textAlign: "center", padding: 24 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>No transactions in {thisAnalytics.label}</div>
                  <div style={{ fontSize: 11, color: T.sub }}>Nothing was logged for this month.</div>
                </div>
              );
            }
            return (
              <div style={{ ...card, marginBottom: 12 }}>
                <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 10 }}>
                  Transactions · {thisAnalytics.label} ({monthTxs.length})
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 360, overflowY: "auto" }}>
                  {[...monthTxs].sort((a, b) => (b.date || "").localeCompare(a.date || "")).map((t, i) => {
                    const isFixed = isMandatoryTx(t);
                    return (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 8px", borderRadius: 8, background: T.card2 }}>
                        <CategoryIcon name={t.category} size={14} color={isFixed ? T.purp : T.acc} />
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 12, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {t.notes || t.category}
                            {isFixed && <span style={{ fontSize: 9, color: T.purp, fontWeight: 700, marginLeft: 6 }}>FIXED</span>}
                          </div>
                          <div style={{ fontSize: 10, color: T.sub }}>
                            {t.date} · {t.category}{t.payment ? ` · ${t.payment}` : ""}
                          </div>
                        </div>
